...
def minibatch_parse(sentences, model, batch_size):
    ...
    while partial_parses:
        sample = partial_parses[:min(batch_size, len(partial_parses))]
        action = model.predict(sample)
        for i in range(len(action)):
            sample[i].parse([action[i]])
            if len(sample[i].buffer) == 0 and len(sample[i].stack) == 1:
                partial_parses.remove(sample[i])
        [dependencies.append(sentence.dependencies) for sentence in sample]
