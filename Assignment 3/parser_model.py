...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        ###        with default parameters.
        ###
        self.embed_to_hidden_weight = nn.Linear(self.embed_size * self.n_features, self.hidden_size, bias=True)
        torch.nn.init.xavier_uniform_(self.embed_to_hidden_weight.weight)

        # self.embed_to_hidden_bias = nn.Linear(1, self.hidden_size)
        # torch.nn.init.uniform_(self.embed_to_hidden_bias.weight)

        self.dropout = nn.Dropout(dropout_prob)

        self.hidden_to_logits_weight = nn.Linear(self.hidden_size, self.n_classes, bias=True)
        torch.nn.init.xavier_uniform_(self.embed_to_hidden_weight.weight)

        # self.hidden_to_logits_bias = nn.Linear(1, self.n_classes)
        # torch.nn.init.uniform_(self.embed_to_hidden_bias.weight)
    ...
    def embedding_lookup(self, w):
        ...
        # return torch.index_select(self.embeddings, w)
        return self.embeddings[w].view(-1, self.n_features*self.embed_size)
    ...
    def forward(self, w):
        ...
        x1 = self.embed_to_hidden_weight(self.embedding_lookup(w))
        x2 = self.dropout(x1)
        x3 = self.hidden_to_logits_weight(x2)
        return x3
